---
title: Web-Push in Adobe Journey Optimizer
description: Erfahren Sie, wie Sie Web-Push-Nachrichten in Adobe Journey Optimizer senden
user-guide-title: Senden von Push-Nachrichten mit Campaign und Journey in Adobe Journey Optimizer
user-guide-description: Dieses Tutorial führt Sie durch die Schritte, die zum Senden von Web-Push-Nachrichten in Adobe Journey Optimizer mithilfe von Kampagnen und Journey erforderlich sind
solution: Journey Optimizer
role: Developer
level: Beginner
breadcrumb-url: /docs/journey-optimizer-learn/ajo-web-push/introduction
source-git-commit: 676c21ca09e0df8d404b05081d71b147755d65d5
workflow-type: tm+mt
source-wordcount: '76'
ht-degree: 7%

---


# Web-Push in Adobe Journey Optimizer{#ajo-web-push}

+ [Einführung](./introduction.md)
+ [Push-Konfiguration erstellen](./create-push-configuration.md)
+ [Datenstrom erstellen](./create-data-stream.md)
+ [Lokales Setup](./run-application-locally.md)
+ [Benutzerberechtigung erfassen](./capture-user-persmission.md)
+ [Zielgruppe erstellen](./create-audience.md)
+ [Kampagne erstellen](./create-campaign.md)
+ [Tag-Eigenschaft erstellen](./create-tag-property.md)
+ [Journey erstellen](./create-journey.md)
+ [Debugging](./debugging.md)

