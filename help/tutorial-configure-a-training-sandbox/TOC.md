---
user-guide-title: Tutorial – Konfigurieren einer Trainings-Sandbox
user-guide-description: Konfigurieren einer Sandbox für Trainingszwecke
breadcrumb-title: Tutorial – Konfigurieren einer Trainings-Sandbox
breadcrumb-url: /docs/journey-optimizer-learn/configure-a-training-sandbox/introduction-and-prerequisites.html
index: false
role: Admin
level: Beginner
auto-video-transcripts: true
source-git-commit: 007d349d95e4fa2a84dfb4f0830fba8af4cf106b
workflow-type: tm+mt
source-wordcount: '63'
ht-degree: 84%

---


# Tutorial – Konfigurieren einer Trainings-Sandbox {#configure-a-training-sandbox}

+ [Konfigurieren einer Trainings-Sandbox](/help/tutorial-configure-a-training-sandbox/introduction-and-prerequisites.md)
+ Schritt 1: Einrichten der Datenstruktur und Importieren von Beispieldaten {#set-up-data-structure-and-ingest-data}
   + [Einrichten der Datenstruktur und Importieren von Beispieldaten mit Postman](https://experienceleague.adobe.com/docs/platform-learn/tutorials/import-sample-data.html?lang=de)
   + [Einrichten der Datenstruktur (manuell)](/help/tutorial-configure-a-training-sandbox/manual-data-set-up.md)
   + [Beispieldaten importieren (manuell)](/help/tutorial-configure-a-training-sandbox/manual-data-ingestion.md)
+ [Schritt 2: Konfigurieren von Ereignissen](/help/tutorial-configure-a-training-sandbox/configure-events.md)
+ [Tutorials](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/overview.html?lang=de)
+ [Herausforderungen](https://experienceleague.adobe.com/docs/journey-optimizer-learn/challenges/introduction-and-prerequisites.html?lang=de)
